...
; ---- POST-CATCH MENU (TAP BASED) ----
DoPostCatchMenu := true

MenuLeadInMs := 300       ; pause BEFORE sequence starts
MenuTapMs    := 30        ; how long key is tapped (SHORT)
MenuGapMs    := 220       ; pause AFTER each tap (MAIN TUNING)
MenuTailMs   := 400       ; pause AFTER sequence ends
; ==============================================
...
gMain.Title := "Auto Fisher — Giant Bait"

; Header style
gMain.SetFont("s11 bold cFFD6E0", "Segoe UI")
gMain.AddText("w260", "Status (Giant Bait):")
...
gMain.AddText("w260 y+10", "Hotkeys: P Start | O Stop | M Exit")
...
PostCatchMenu() {
    global DoPostCatchMenu, MenuLeadInMs, MenuTailMs
    if !DoPostCatchMenu
        return
